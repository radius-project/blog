...
- **Native GitOps integration** with Flux enables teams to promote application and infrastructure updates from Git while Radius keeps the desired state synchronized. The integration includes a new Flux Controller that watches for changes in Git repositories containing and a new Deployment Template Controller that creates, updates, or deletes applications based on these configurations. This brings the operational benefits of GitOps workflows while preserving Radius's application-centric approach.

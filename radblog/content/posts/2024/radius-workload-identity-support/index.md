...
Radius allows management of AWS resources as part of your application. There are two Radius services that communicate with AWS to achieve this - UCP and Applications RP.
...
6. ucp (applications-rp) uses this temporary credentials to make suitable API requests to manage the AWS resources. 
...
[Radius Setup AWS IRSA](https://docs.radapp.io/guides/operations/providers/aws-provider/howto-aws-provider-irsa/) contains information on configuring the trust for the IAM Roles in AWS.
...
6. ucp (applications-rp or bicep-de) uses this temporary credentials to make suitable API requests to mange the Azure resources. 
...
AWS and Azure provide very similar solutions for adopting workload identities. They also provide mutating webhooks that allow configuring workloads to use Workload Identity easily. However, Azure's mutating webhook supports its usage to just mount the service token account where as with AWS, it was not easy to do so.  This was because Azure's webhook utilized two annotations as part of workload identity configuration. We could choose to utilize ```azure.workload.identity/use: "true"``` to mount the service-token which is the key requirement for enabling workload identity.

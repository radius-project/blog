...
Up until now, Radius has used a temporary fork of Bicep to add support for resources that are unique to Radius. Bicep uses type definitions to handle Azure resources, as outlined in the [Azure Bicep types repository](https://github.com/Azure/bicep-types-az). The type definitions provide information about the properties and configurations of resources already built into Bicep and not for other resources. We needed to be able to ship Radius, Kubernetes, and AWS resource type definitions separately from Bicep so that they could be easily updated based on changing needs. 
